...
export async function processStarboardReaction(guildId, channelId, messageId, emoji, userId) {
  try {
    ...
    // Check if this is the starboard emoji
	  function emojiMatches(eventEmoji, configEmoji) {
		  if (!eventEmoji.id) {
		    return eventEmoji.name === configEmoji
		  }

		  const fullCustomEmoji = `<:${eventEmoji.name}:${eventEmoji.id}>`
		  return fullCustomEmoji === configEmoji
		}
	  const configEmoji = "⭐"
	  if (!emojiMatches(emoji, configEmoji)) return false
    // Get the message
