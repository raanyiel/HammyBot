...
// Storage for GitHub webhook configurations
// Format: Map<guildId, Map<repositoryName, {channelId, events}>>
const githubWebhooks = new Map()
...
export function setGithubWebhook(guildId, repository, channelId, events = "all") {
  if (!githubWebhooks.has(guildId)) {
    githubWebhooks.set(guildId, new Map())
  }

  const guildWebhooks = githubWebhooks.get(guildId)
  guildWebhooks.set(repository, { channelId, events })

  return true
}

// Remove a GitHub webhook for a repository
export function removeGithubWebhook(guildId, repository) {
  if (!githubWebhooks.has(guildId)) {
    return false
  }

  const guildWebhooks = githubWebhooks.get(guildId)
  return guildWebhooks.delete(repository)
}

// List all GitHub webhooks for a guild
export function listGithubWebhooks(guildId) {
  if (!githubWebhooks.has(guildId)) {
    return []
  }

  const guildWebhooks = githubWebhooks.get(guildId)
  return Array.from(guildWebhooks.entries()).map(([repo, config]) => ({
    repository: repo,
...
export function findGithubWebhook(repository) {
  for (const [guildId, guildWebhooks] of githubWebhooks.entries()) {
    for (const [repo, config] of guildWebhooks.entries()) {
